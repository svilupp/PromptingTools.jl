...
### Added
- Added support for OpenAI's GPT-5.5 model (`gpt-5.5`, alias `gpt55`). GPT-5.5 is OpenAI's latest flagship model for complex tasks like coding, analysis, and planning. 1M context, 128K output.
- Added support for Anthropic's Claude Sonnet 5 model (`claude-sonnet-5`, alias `claude5`). Claude Sonnet 5 is the latest Sonnet-class model balancing intelligence and speed with 1M context and 128K output. The `claude` and `claudes` aliases now point to this model.
- Added support for Anthropic's Claude Fable 5 model (`claude-fable-5`, alias `claudef`). Claude Fable 5 is Anthropic's most capable model for demanding reasoning and long-horizon agentic work, with 1M context and 128K output.
