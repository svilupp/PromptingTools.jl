...
dir_raw = joinpath("markdown", "DataFrames") # folder with text documents
files = ["comparison_with_python.txt", "database_style_joins.txt", "what_is_dataframes.txt"]
index = build_index(joinpath.(dir_raw, files); extract_metadata = false)
...
# Note: In practice, you would review each item in this golden evaluation set (and delete any generic/poor questions). 
...
# Fortunately, we don't have to do this one by one -- let's evaluate all out Q&A pairs at once.
...
# Note that failed evals can show as "nothing", so make sure to handle them
