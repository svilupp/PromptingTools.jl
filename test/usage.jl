...
@testset "get_cache_discounts" begin
    ...
    # Prefix matching
    discounts = get_cache_discounts("gpt-4o-2024-08-06")
    @test discounts.read_discount == 0.5

    # Model family inference
    discounts = get_cache_discounts("claude-sonnet-4-20250514")
    @test discounts.read_discount == 0.9
    @test discounts.write_premium == 0.25

    discounts = get_cache_discounts("gemini-2.5-flash")
    @test discounts.read_discount == 0.9

    # Unknown model
    discounts = get_cache_discounts("unknown-model")
