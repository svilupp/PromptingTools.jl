...
For more information on templates, see the [Templated Prompts](/examples/readme_examples#templated-prompts) section.
...
See the docs for more complex examples and usage tips (`?airetry`). We leverage Monte Carlo Tree Search (MCTS) to optimize the sequence of retries, so it's a very powerful tool for building robust AI workflows (inspired by [Language Agent Tree Search paper](https://arxiv.org/abs/2310.04406) and by [DSPy Assertions paper](https://arxiv.org/abs/2312.13382)).
...
If you're getting errors, check that Ollama is running - see the [Setup Guide for Ollama](/examples/readme_examples#setup-guide-for-ollama) section below.
