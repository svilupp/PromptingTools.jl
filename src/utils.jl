...
"""
const CACHE_DISCOUNTS = Dict{String, NamedTuple{(:read_discount, :write_premium), Tuple{Float64, Float64}}}(
...
"""
    get_cache_discounts(model_id::String) -> NamedTuple{(:read_discount, :write_premium), Tuple{Float64, Float64}}

Get cache discount configuration for a model. Uses prefix matching and model family inference.
...
get_cache_discounts("claude-sonnet-4-20250514")  # (read_discount = 0.9, write_premium = 0.25)
get_cache_discounts("unknown-model")  # (read_discount = 0.0, write_premium = 0.0)
```
"""
function get_cache_discounts(model_id::String)
    # Try exact match first
    haskey(CACHE_DISCOUNTS, model_id) && return CACHE_DISCOUNTS[model_id]

    # Try prefix matching
    for (prefix, discounts) in CACHE_DISCOUNTS
        startswith(model_id, prefix) && return discounts
    end

    # Infer from model family patterns
    model_lower = lowercase(model_id)
    if occursin("claude", model_lower)
        return (read_discount = 0.9, write_premium = 0.25)
    elseif occursin("gpt-5", model_id) || occursin("gpt5", model_id)
        return (read_discount = 0.9, write_premium = 0.0)
    elseif occursin("gpt-4", model_id) || occursin("gpt4", model_id)
        return (read_discount = 0.5, write_premium = 0.0)
    elseif occursin("gemini", model_lower)
        return (read_discount = 0.9, write_premium = 0.0)
    elseif startswith(model_id, "o1") || startswith(model_id, "o3")
        return (read_discount = 0.5, write_premium = 0.0)
    end
