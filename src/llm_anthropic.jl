...
function render(schema::AbstractAnthropicSchema,
    ...
    @assert length(conversation)==0 "AbstractAnthropicSchema does not allow providing past conversation history. Use the `prompt` argument or `ChatMLSchema`."
    ...
    for msg in messages_replaced
        ...
        elseif msg isa UserMessageWithImages
            error("AbstractAnthropicSchema does not support UserMessageWithImages. Please use OpenAISchema instead.")
        end
...
function anthropic_api(
    ...
    ## 
    headers = auth_header(
        api_key; x_api_key = true, extra_headers = ["anthropic-version" => "2023-06-01"])
...
"""
    aiembed(prompt_schema::AbstractAnthropicSchema,
...
## Arguments
- `prompt_schema::AbstractAnthropicSchema`: The schema for the prompt.
...
function aiembed(prompt_schema::AbstractAnthropicSchema,
